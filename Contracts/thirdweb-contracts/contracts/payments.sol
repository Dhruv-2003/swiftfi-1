...
contract payments is Ownable {
    ...
    uint256 public interestPerMonthEMI = 15;
    ...
    uint256 public interestRateOnPL = 10;
    ...
    function payViaWallet(
        ...
        address payee = _manager.getWalletContract(sender);
    ...
    function completePayStream(uint256 _streamID) public onlyApproved {
        Stream storage _streamer = Streams[_streamID];
        require(
    ...
    function createPLRequest(
        ...
        require(
            profileWallet(wallet).getBalance() >= _amount,
    ...
    function payPlRequest(uint256 _plID) public payable {
        plRequest storage _plrequest = plRequests[_plID];
        require(
    ...
    function calculatePaybackAmount(uint256 _plID)
        public
        view
        returns (uint256)
    {
        ...
        uint256 interestCharged = (_plrequest.amount *
            timeElapsed *
            (interestRateOnPL / 2628000)) / 100;
        uint256 paybackAmount = _plrequest.amount + interestCharged;

        return paybackAmount;
    }

    function checkPlRequest(uint256 _plId) public view returns (bool) {
        plRequest storage _plrequest = plRequests[_plId];
    ...
    function getPlRequest(uint256 _plID)
        public
        view
        returns (plRequest memory)
    {
    ...
    function payEMI(uint256 _EMIId) public payable {
        ...
        require(success, "Payment not completed");
        address wallet = _manager.getWalletContract(_emi.sender);
    ...
    function payEMIwithDefaults(uint256 _EMIId) public payable {
        ...
        uint256 newPerMonthAmount = (_emi.perMonth +
            ((_emi.perMonth * (interestPerMonthEMI / 12)) / 100));
    ...
    function checkEMIDefault(uint256 _EMIId) public {
        ...
        if (_emi.timePaid == monthPassed) {
            _emi.defaults = 0;
        } else if (_emi.timePaid < monthPassed) {
            _emi.defaults = monthPassed - _emi.timePaid;
        } else {
            _emi.defaults = 0;
        }
    }
